...
export async function startCamAR(
  ...
  const depthFar = Math.min(6, Math.max(1, parseFloat(localStorage.getItem('oheya:depth') || '3')));
  field.spawnAt(new THREE.Vector3(-0.9, 0, Math.random() * 0.6 - 0.4), tierRoll(Math.random));
  field.spawnAt(new THREE.Vector3(0.9, 0, Math.random() * (depthFar * 0.8) - depthFar * 0.8), tierRoll(Math.random));
  field.spawnAt(new THREE.Vector3(Math.random() * 0.6 - 0.3, 0, Math.random() * 0.5 - depthFar), tierRoll(Math.random));
  field.spawnAt(new THREE.Vector3(Math.random() * 0.8 - 0.4, 0, 0.6), tierRoll(Math.random));
  ...
  field.renderer.setAnimationLoop((now) => {
    ...
    if (cooldown <= 0 && field.activeCount() < field.maxCount()) {
      ...
      const z = Math.random() * (depthFar + 0.6) - depthFar; // 手前〜奥(depthFar)まで様々な距離感で出現
